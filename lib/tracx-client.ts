...
export class TracxLogisClient {
  ...
  async trackShipment(trackingNumber: string): Promise<TrackingResult> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`TracxLogis API error: ${response.status} - ${errorText}`);
      }
  ...
  async trackMultiple(trackingNumbers: string[]): Promise<TrackingResult[]> {
    try {
      const response = await fetch(
        `${this.baseUrl}/v2/tracking/multi`,
        {
          method: 'POST',
          headers: this.getHeaders(),
   ...
   */
  private parseTrackingResponse(trackingNumber: string, data: Record<string, unknown>): TrackingResult {
    const events: TrackingEvent[] = [];
    
    // Parse events/checkpoints from response
    const checkpoints = (data?.checkpoints || data?.events || data?.tracking_history || []) as Array<Record<string, unknown>>;
    
    for (const checkpoint of checkpoints) {
      events.push({
        timestamp: String(checkpoint.time || checkpoint.timestamp || checkpoint.date || ''),
        status: String(checkpoint.status || checkpoint.tag || ''),
        statusCode: String(checkpoint.status_code || checkpoint.tag || ''),
        location: String(checkpoint.location || checkpoint.city || checkpoint.country || ''),
        description: String(checkpoint.message || checkpoint.description || checkpoint.status || ''),
      });
    }
 ...
 */
export async function syncShipmentStatus(trackingNumber: string): Promise<TrackingResult | null> {
  ...
  await prisma.shipment.update({
    ...
    data: {
      ...
      actualDelivery: result.actualDelivery ? new Date(result.actualDelivery) : undefined,
      deliveredAt: internalStatus === 'DELIVERED' && result.actualDelivery 
        ? new Date(result.actualDelivery) 
        : undefined,
      pickedUpAt: internalStatus === 'PICKED_UP' || ['IN_TRANSIT', 'OUT_FOR_DELIVERY', 'DELIVERED'].includes(internalStatus)
        ? result.events.find(e => mapTracxStatus(e.statusCode) === 'PICKED_UP')?.timestamp 
          ? new Date(result.events.find(e => mapTracxStatus(e.statusCode) === 'PICKED_UP')!.timestamp)
          : undefined
        : undefined,
    },
  });
 ...
 */
export async function syncAllActiveShipments(): Promise<{ synced: number; errors: number }> {
