...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.channel || !ORDER_CHANNELS.includes(body.channel)) {
      return NextResponse.json(
        { success: false, error: `Channel must be one of: ${ORDER_CHANNELS.join(', ')}` },
        { status: 400 }
      );
    }
    ...
    for (const item of body.items) {
      if (!item.productName || !item.sku || !item.quantity || item.unitPrice === undefined) {
        return NextResponse.json(
          { success: false, error: 'Each item must have productName, sku, quantity, and unitPrice' },
          { status: 400 }
        );
      }
      if (item.quantity <= 0) {
