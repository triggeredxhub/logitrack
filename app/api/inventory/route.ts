...
export async function GET(request: NextRequest) {
  try {
    ...
    if (expiredOnly) {
      where.OR = [{ isExpired: true }, { expiryDate: { lt: now } }];
    ...
    const batchesWithDetails = batches.map((batch) => {
      ...
      return {
        ...
        costPrice: batch.costPrice
          ? parseFloat(batch.costPrice.toString())
          : null,
        product: {
          ...batch.product,
          sellingPrice: batch.product.sellingPrice
            ? parseFloat(batch.product.sellingPrice.toString())
            : null,
        },
        daysUntilExpiry,
        isExpiringSoon,
  ...
  try {
    ...
    const batch = await prisma.inventoryBatch.create({
      ...
      include: {
        ...
        // 🔥 2. SEND STOCK TO TRACX (MOVE HERE)
    if (batch.product?.tracxSkuNo) {
      const tracxClient = new TracxLogisClient({
        apiKey: process.env.TRACX_API_KEY!,
        apiEndpoint: "https://api.tracxlogis.com",
      });

      await tracxClient.sendStock({
        tracxSkuNo: batch.product.tracxSkuNo,
        quantity: batch.quantity,
        warehouse: batch.location ?? undefined,
      });
    }
