...
export async function GET(request: NextRequest) {
  console.log("🚀 Shopee callback HIT");
  ...
  try {
    ...
    if (!code || !shopId) {
      return NextResponse.redirect(
        new URL(
          "/dashboard/data-sync?error=missing_params",
    ...
    if (!config) {
      return NextResponse.redirect(
        new URL(
          "/dashboard/data-sync?error=config_not_found",
          process.env.NEXTAUTH_URL || "",
        ),
      );
    }
    ...
    if (tokenResponse.error || !tokenResponse.response) {
      console.error(
        ...
        tokenResponse.message || tokenResponse.error,
      );
      return NextResponse.redirect(
        new URL(
          `/dashboard/data-sync?error=token_exchange_failed&message=${encodeURIComponent(tokenResponse.message || "Unknown error")}`,
          process.env.NEXTAUTH_URL || "",
        ),
      );
    }
    ...
    const expiresAt = new Date(
      Date.now() + tokenResponse.response.expire_in * 1000,
    );
    ...
    return NextResponse.redirect(
      new URL(
        "/dashboard/data-sync?success=authorized",
        process.env.NEXTAUTH_URL || "",
      ),
    );
  } catch (error) {
    ...
    return NextResponse.redirect(
      new URL(
        "/dashboard/data-sync?error=callback_failed",
        process.env.NEXTAUTH_URL || "",
      ),
    );
  }
